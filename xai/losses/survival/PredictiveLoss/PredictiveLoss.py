...
## Create predictive biomarker loss function
...
def PredictiveBiomarkerLoss(y_true, y_pred, omega=0.5):
    ...
    ind_approx_1 = 1.0/(1.0+tf.exp(-beta / omega)) # g1
    ind_approx_2 = 1.0/(1.0+tf.exp(beta / omega)) # g2
    
    treatment_A = tf.cast(y_true[:,3] == 1, tf.float32) # Treatment == 1 (A)
    treatment_B = tf.cast(y_true[:,3] == 0, tf.float32) # Treatment == 0 (B)
    ...
    S1 = (g1_num_A*g1_num_A / (g1_denom_A+1.0)) + (g2_num_A*g2_num_A / (g2_denom_A+1.0))
    S2 = (g2_num_B*g2_num_B / (g2_denom_B+1.0)) + (g1_num_B*g1_num_B / (g1_denom_B+1.0))
    
    S3 = (g1_num_A*g1_num_A / (g1_denom_A+1.0)) + (g1_num_B*g1_num_B / (g1_denom_B+1.0))
    S4 = (g2_num_A*g2_num_A / (g2_denom_A+1.0)) + (g2_num_B*g2_num_B / (g2_denom_B+1.0))
    

    loss = 1*(tf.math.log(S2) - tf.math.log(S1))
